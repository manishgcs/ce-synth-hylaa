...
def define_ha(settings, args):
    ...
    if usafe_r is None:
        usafe_set_constraint_list.append(LinearConstraint([1.0, 0.0, 0.0], -1.0))
    else:
...
if __name__ == '__main__':
    ...
    a_matrix = np.array([[2.2553, -1], [1, 0]], dtype=float)
    b_matrix = np.array([[0.5], [0]], dtype=float)
