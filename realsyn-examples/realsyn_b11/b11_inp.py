...
def define_ha(settings, args):
    ...
    if usafe_r is None:
        # usafe_set_constraint_list.append(LinearConstraint([1.0, 0.0, 0.0, 0.0], -4.0))
        usafe_set_constraint_list.append(LinearConstraint([-1.0, 0.0, 0.0, 0.0], -4.0))
...
if __name__ == '__main__':
    ...
    a_matrix = np.array([[2.6207, -1.1793, 0.65705, 0], [2, 0, 0, 0], [0, 0.5, 1, 0], [0, 0, 0, 1]], dtype=float)
    b_matrix = np.array([[8], [0], [0], [0]], dtype=float)

    dim = len(b_matrix)
    l_idx = 0
    k_matrix = []
    controller_f_name = '/home/manishg/Research/realsyn/controller_outputs/' + str(11) + '_controller.txt'
