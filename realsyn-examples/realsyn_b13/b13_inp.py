...
def define_ha(settings, args):
    ...
    ha.variables = ["x1", "x2", "t"]

    locations = []
    n_locations = len(step_inputs)-6
    ...
    if usafe_r is None:
        usafe_set_constraint_list.append(LinearConstraint([1.0, 0.0, 0.0], -1.5))
        # usafe_set_constraint_list.append(LinearConstraint([-1.0, 0.0, 0.0], 0.5))
    else:
...
if __name__ == '__main__':
    ...
    a_matrix = np.array([[0.98965, 1.4747e-08, 0], [7.4506e-09, 0, 0], [0, 0, 1]], dtype=float)
    b_matrix = np.array([[128], [0], [0]], dtype=float)

    dim = len(b_matrix)
    l_idx = 0
    k_matrix = []
    controller_f_name = '/home/manishg/Research/realsyn/controller_outputs/' + str(13) + '_controller.txt'
