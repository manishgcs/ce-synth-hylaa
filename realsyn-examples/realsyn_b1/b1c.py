...
def define_ha(settings, usafe_r):
    ...
    # c_matrix = np.ones(2)
    # print(np.matmul(c_matrix, np.matmul(np.linalg.inv(a_bk_matrix), b_matrix)))
    # loc1.c_vector = 10*b_matrix.flatten()
    ...
    # d_vector = np.array([5, 5], dtype=float)
    # local_c_vector = -np.matmul(np.matmul(b_matrix, k_matrix), d_vector)
    # print(local_c_vector)
    # loc1.c_vector = local_c_vector

    # k_vector = np.array([[1.5359, 1.9282]], dtype=float)
    # k_vector = np.array([[1.4416, -1.078]], dtype=float)

    # a_bk_inverse = np.linalg.inv(-a_bk_matrix)
    # f_denom = np.matmul(np.eye(2), np.matmul(a_bk_inverse, b_matrix))
    # F_vector = 1/f_denom
    ...
    if usafe_r is None:
        ...
        # usafe_set_constraint_list.append(LinearConstraint([0.0, 1.0], -0.25))
        usafe_set_constraint_list.append(LinearConstraint([1.0, 0.0], -0.1))
    else:
