...
def define_ha(settings, args):
    ...
    for idx in range(n_locations):
        ...
        loc.inv_list.append(LinearConstraint([0.0, 0.0, 0.0, 1.0], step_size*idx))  # t <= 0.1
        locations.append(loc)

    for idx in range(n_locations-1):
        trans = ha.new_transition(locations[idx], locations[idx+1])
        trans.condition_list.append(LinearConstraint([-0.0, -0.0, 0.0, -1.0], -step_size*idx))  # t >= 0.1
    ...
    if usafe_r is None:
        usafe_set_constraint_list.append(LinearConstraint([1.0, 0.0, -0.0, 0.0], -3.0))
        # usafe_set_constraint_list.append(LinearConstraint([-1.0, 0.0, 1.0, 0.0], 0.5))
    else:
...
if __name__ == '__main__':
    ...
    a_matrix = np.array([[5.5197e-17, -3.5503e-17, 6.2468e-32, 0], [2.7756e-17, 0, 0, 0], [0, 2.7756e-17, 1, 0], [0, 0, 0, 1]], dtype=float)
    b_matrix = np.array([[0.25], [0], [0], [0]], dtype=float)

    dim = len(b_matrix)
    l_idx = 0
    k_matrix = []
    controller_f_name = '/home/manishg/Research/realsyn/controller_outputs/' + str(19) + '_controller.txt'
