...
def define_ha(settings, args):
    ...
    if usafe_r is None:
        ...
        # usafe_set_constraint_list.append(LinearConstraint([0.0, -1.0], -1.7))
        # usafe_set_constraint_list.append(LinearConstraint([0.0, 1.0], 2.0))
    else:
...
if __name__ == '__main__':
    ...
    init_r = HyperRectangle([(3.0, 4.0), (0, 0)])
    usafe_r = None

    a_matrix = np.array([[0.99501]], dtype=float)

    b_matrix = np.array([[0.0078125]], dtype=float)

    a_matrix_ext, b_matrix_ext = extend_a_b(a_matrix, b_matrix)

    Q1 = np.array([[1]], dtype=float)
